...
from utils.loss import ComputeLoss, compute_distillation_loss
...
import val_nas  # for end-of-epoch mAP
import argparse
import logging
import math
import os
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, unfreeze_layer = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.unfreeze_layer
    ...
    if pretrained:
        ...
        if 'model' not in ckpt.keys():
            csd = torch.load(weights, map_location=device)
            LOGGER.info(f'Load convert model')  # report
        else:
    ...
    if opt.distill:
        LOGGER.info(f"load teacher-model from {opt.t_weights}")
    ...
    # if opt.split:
    #     # unFreeze
    #     freeze = ['model.'+str(unfreeze_layer)+'.']
    #     for k, v in model.named_parameters():
    #         if any(x in k for x in freeze):
    ...
    if pretrained and 'optimizer' in ckpt.keys():
        # Optimizer
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            seeds = i % 4
            model.set_subnet(seeds)
            # print(seeds)

            # Forward
            with amp.autocast(enabled=cuda):
                
                pred, loss_feat = model(imgs)  # forward
                ...
                # distillation
                if opt.distill:
                    dloss = compute_distillation_loss(
                        pred, t_pred, model, opt.dist_loss, opt.temperature)
                    loss += dloss

                # if opt.split and not opt.only_det:
                #     if opt.only_autoencoder:
                #         loss = loss_feat
                #     else:
                #         loss += loss_feat
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val_nas.run(data_dict,
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val_nas.run(data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
                                            model=attempt_load(
                                                f, device).half(),
                                            ...
                                            callbacks=callbacks,
                                            compute_loss=compute_loss,
                                            split=opt.split)  # val best model with plots
                    if is_coco:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--distill', action='store_true',
                        help='Whether to use knowledge distillation training')
    parser.add_argument('--t_weights', type=str,
                        default='yolov5l.pt', help='Teacher weight file')
    parser.add_argument('--dist_loss', type=str, default='kl',
                        help='using kl/l2 loss in distillation')
    parser.add_argument('--temperature', type=int, default=5,
                        help='temperature in distilling training')
    parser.add_argument('--a', type=float, default=0.3,
                        help='Ratio of distillation loss to total loss')
    # kd --------------------------------------------------
