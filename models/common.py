...
class DeConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        # if p is None:
        #     p = (k - 1) // 2  # This is a simple autopadding calculation for symmetrical padding
        # self.deconv = nn.ConvTranspose2d(c1, c2, k, s, p, output_padding,
        #                                          groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (
            act if isinstance(act, nn.Module) else nn.Identity())
...
class DeconvC3(nn.Module):
    # Inverted CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = DeConv(c1, c_, 1, 1)
        self.cv2 = DeConv(c1, c_, 1, 1)
        self.cv3 = DeConv(2 * c_, c2, 1)  # adjust filters appropriately
...
class DeconvSPPF(nn.Module):
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = DeConv(c1, c_, 1, 1)
        self.cv2 = DeConv(c_ * 4, c2, 1, 1)
        self.m = nn.ConvTranspose2d(c_, c_, k, 1, k // 2)
